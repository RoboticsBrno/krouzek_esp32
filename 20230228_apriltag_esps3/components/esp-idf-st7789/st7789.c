...
esp_err_t spi_master_write_byte(spi_device_handle_t SPIHandle, const uint8_t* Data, size_t DataLength)
{
...
// Add 202001
esp_err_t spi_master_write_colors(TFT_t * dev, uint16_t *colors, uint32_t size)
{
	uint8_t *colors8 = (uint8_t*)colors;
	uint8_t buffer[1024];
	size_t offset = 0;
	...
	while(size > 0) {
		...
		for(uint32_t i = 0; i < chunk; i+=2) {
			buffer[i] = colors8[offset+i+1];
			buffer[i+1] = colors8[offset+i];
		}

		auto err = spi_master_write_byte( dev->_SPIHandle, buffer, chunk);
		if (err != ESP_OK) {
			return err;
		}
...
// colors:colors
void lcdDrawMultiPixels(TFT_t * dev, uint16_t x, uint16_t y, uint32_t size, uint16_t * colors) {
	...
	spi_master_write_command(dev, 0x2C);	//	Memory Write
	ESP_ERROR_CHECK(spi_master_write_colors(dev, colors, size));
}
